...
namespace LiveCaptionsTranscriber.utils
{
    ...
    {
        public static readonly string CONNECTION_STRING = "Data Source=transcription_history.db;";
        ...
        public static async Task LogTranscription(string sourceText, string translatedText,
            string targetLanguage, string apiUsed, CancellationToken token = default)
        {
            string insertQuery = @"
                INSERT INTO TranscriptionHistory (Timestamp, SourceText, TranslatedText, TargetLanguage, ApiUsed)
        ...
        public static async Task<(List<TranscriptionHistoryEntry>, int)> LoadHistoryAsync(
            int page, int maxRow, string searchText, CancellationToken token = default)
        {
            var history = new List<TranscriptionHistoryEntry>();
            int totalCount = 0;
            using (var command = new SqliteCommand(@"
                SELECT COUNT(*) 
        ...
        {
            string selectQuery = "DELETE FROM TranscriptionHistory; DELETE FROM sqlite_sequence WHERE NAME='TranscriptionHistory'";
        ...
        public static async Task<TranscriptionHistoryEntry?> LoadLastTranscription(CancellationToken token = default)
        {
            string selectQuery = @"
                SELECT Timestamp, SourceText, TranslatedText, TargetLanguage, ApiUsed
                FROM TranscriptionHistory
        ...
        public static async Task DeleteLastTranscription(CancellationToken token = default)
        {
            using (var command = new SqliteCommand(@"
                DELETE FROM TranscriptionHistory
                WHERE Id IN (SELECT Id FROM TranscriptionHistory ORDER BY Id DESC LIMIT 1)",
                GetConnection()))
            {
                await command.ExecuteNonQueryAsync(token);
            }
        ...
        private static async Task MigrateOldTimestampFormat()
        {
            ...
            using (var command = new SqliteCommand("SELECT Id, Timestamp FROM TranscriptionHistory", GetConnection()))
            ...
            foreach (var (id, timestamp) in records)
            {
                if (DateTime.TryParse(timestamp, out DateTime dt))
                {
                    ...
                    using var updateCommand = new SqliteCommand(
                        "UPDATE TranscriptionHistory SET Timestamp = @Timestamp WHERE Id = @Id",
                        GetConnection());
